package com.xlsoft.kudanar.FocusBox;
...
public class FocusBoxView extends View {
    ...
    private static final int MIN_FOCUS_BOX_HEIGHT = 100;
     int MAX_FOCUS_BOX_WIDTH ;
     int MAX_FOCUS_BOX_HEIGHT ;
   ...
   public void setdisplay(Display d){
    ...
    private void updateBoxRect(int dW, int dH) {
        ...
        if (newWidth < MIN_FOCUS_BOX_WIDTH || newHeight < MIN_FOCUS_BOX_HEIGHT)
            return;
        MAX_FOCUS_BOX_WIDTH=display.getWidth()-80;
        MAX_FOCUS_BOX_HEIGHT=display.getHeight()/3;

        if (newWidth > MAX_FOCUS_BOX_WIDTH || newHeight > MAX_FOCUS_BOX_HEIGHT)
            return;
// max size is to be set out here...

        box = new Rect(leftOffset, topOffset, leftOffset + newWidth, topOffset + newHeight);
        //from here coordinates would likely to be found out.
    ...
    public void onDraw(Canvas canvas) {
        ...
        //set max window size....
            ...
            canvas.drawCircle(frame.left, frame.top, 22, paint);
            canvas.drawCircle(frame.right, frame.top, 22, paint);
            canvas.drawCircle(frame.left, frame.bottom, 22, paint);
            canvas.drawCircle(frame.right, frame.bottom, 22, paint);
